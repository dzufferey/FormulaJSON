...
%bldexe%  .\JSONBuild\JSONBuild.csproj /t:Clean /p:Configuration=Debug /p:Platform=AnyCPU /verbosity:quiet /nologo
if %ERRORLEVEL% neq 0 (
  echo Could not clean build utility.
  exit /B 1
)

%bldexe%  .\JSONBuild\JSONBuild.csproj /t:Build /p:Configuration=Debug /p:Platform=AnyCPU /verbosity:quiet /nologo
if %ERRORLEVEL% neq 0 (
  echo Could not compile build utility.
  exit /B 1
)

.\JSONBuild\bin\Debug\JSONBuild.exe %1
if %ERRORLEVEL% neq 0 (
  echo Build failed.
  exit /B 1
) else ( exit /B 0 )